...
class ProtobufFunctionsSuite extends QueryTest with SharedSparkSession with ProtobufTestBase
  ...
  test("Zero values should become null in the result when deserialized") {
    // In proto3, there is no distinction between a field having
    // a zero value and a field not being present. This test
    // confirms that behavior and that we treat both the same.
  ...
  test("Zero values should be populated when the materializeZeroValues configuration is set") {
    // In proto3, there is no distinction between a field having
    // a zero value and a field not being present. This test
    // confirms that behavior and that we treat both the same.
      ...
      checkAnswer(
        explicitZeroInput.select(
          from_protobuf_wrapper(
            ...
            name,
            descFilePathOpt,
            Map(ProtobufOptions.materializeZeroValues -> "true")).as("proto")),
        expected)
      checkAnswer(
